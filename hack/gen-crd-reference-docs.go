...
- Copyright 2021 Pelotech
  - Minor adjustments and function additions
*/
...
func render(w io.Writer, pkgs []*apiPackage, config generatorConfig) error {
	...
	t, err := template.New("").Funcs(map[string]interface{}{
		...
		"shortName": func(t *types.Type, depth int) string {
			dn := typeDisplayName(t, config, typePkgMap)
			spl := strings.Split(dn, "/")
			return strings.Join(spl[len(spl)-depth:], "/")
		},
